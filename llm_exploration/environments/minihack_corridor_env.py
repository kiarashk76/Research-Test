"""MiniHack-Corridor family: a thin wrapper around MiniHack's own
...
# The 2-room corridor task -- MiniHack-Corridor's other variants (R3, R5)
# just connect more rooms in a row via longer randomly-generated corridors;
# pass e.g. ``env_id="MiniHack-Corridor-R5-v0"`` as an override to pick a
# different one without any code change here. See
# https://minihack.readthedocs.io/en/latest/envs/navigation/corridor.html
# for the full list.
DEFAULT_CORRIDOR_ENV_ID = "MiniHack-Corridor-R2-v0"

# MiniHack's own built-in per-episode step cap -- enforced by NLE itself
# (truncated=True once reached), not something this wrapper adds. Unlike
# MiniHack-Room's size-scaled default, MiniHackCorridor.__init__
# (minihack/envs/corridor.py) uses one flat default (1000) for every R2/
# R3/R5 variant -- verified directly against the installed package's
# source, not assumed.
def default_max_episode_steps_for(env_id: str) -> int:
    """MiniHackCorridor's real default `max_episode_steps` -- same for
    every registered ``MiniHack-Corridor-*`` id (see module comment
    above); ``env_id`` is accepted only so callers (e.g. the Setup page)
    can use this the same way as the Room family's version of this
...
# for the same reason as MiniHackRoomEnv's identical comment: it would let
# the LLM draw on pretrained knowledge of that specific, extensively-
# documented game instead of learning purely from interaction. Kept
# identical across every Corridor variant (R2/R3/R5) for the same reason --
# the description itself must not reveal the corridor count/layout, which
# has to come from the agent's own observations.
ENVIRONMENT_DESCRIPTION = (
    "A navigation task across multiple connected rooms. The agent starts somewhere "
    "in one room and must find and reach a marked exit, possibly passing through "
    "other rooms and corridors along the way. The layout, and anything else in it, "
...
class MiniHackCorridorEnv(BaseEnvironment):
    """Wraps any ``MiniHack-Corridor-*`` task id as a plain Gymnasium
    ...
    def __init__(self, env_id: str = DEFAULT_CORRIDOR_ENV_ID,
        ...
        # MiniHack-Corridor's default (unlike Room's) is 8-way movement
        # PLUS open/kick/search (minihack/envs/corridor.py's
        # NAVIGATE_ACTIONS) -- doors along the corridor may need opening
        # or kicking open.
...
# Populated per-instance (see MiniHackCorridorEnv.__init__, and
...
# gets plain numeric labels ("0", "1", ...) until a MiniHackCorridorEnv has
# actually been constructed at least once.
ACTION_NAMES: dict[int, str] = {}


def play_minihack_corridor() -> None:
    env = MiniHackCorridorEnv()
    observation, _ = env.reset()
    terminated = False
    truncated = False

    print(f"MiniHackCorridorEnv ({env.env_id})")
