"""MiniHack-ExploreMaze family: a thin wrapper around MiniHack's own
...
# The easiest, non-premapped variant -- every other ExploreMaze variant
# (-Hard-, or either difficulty's "-Mapped-" counterpart, which
# pre-reveals the maze layout instead of requiring it to be explored) is
# just a different registered env id; pass e.g.
# ``env_id="MiniHack-ExploreMaze-Hard-v0"`` as an override to pick a
# different one without any code change here. See
# https://minihack.readthedocs.io/en/latest/envs/skill_hard/exploremaze.html
# for the full list.
DEFAULT_MAZEEXPLORE_ENV_ID = "MiniHack-ExploreMaze-Easy-v0"
...
# MiniHackExploreMaze.__init__ (minihack/envs/exploremaze.py) uses one
# flat default (500) for every registered variant -- verified directly
# against the installed package's source, not assumed.
def default_max_episode_steps_for(env_id: str) -> int:
    """MiniHackExploreMaze's real default `max_episode_steps` -- same for
    every registered ``MiniHack-ExploreMaze-*`` id (see module comment
...
# identical across every ExploreMaze variant (Easy/Hard, mapped or not)
# for the same reason -- the description itself must not reveal what the
# consumable items are or that they're repeatable-reward, which has to
# come from the agent's own observations.
ENVIRONMENT_DESCRIPTION = (
    "A maze-navigation task. The agent starts somewhere in a maze and must find and "
    "reach a marked exit; consumable items encountered along the way may provide "
    "additional reward. The layout, and anything else in it, must be discovered "
    "through interaction."
)
...
class MiniHackMazeExploreEnv(BaseEnvironment):
    """Wraps any ``MiniHack-ExploreMaze-*`` task id as a plain Gymnasium
    ...
    built-in reward manager, which grants repeatable partial reward for
    eating an item and full reward for reaching the stairs down) is
    entirely MiniHack's.
    """

    metadata = {"render_modes": ["ansi", "human"]}

    def __init__(self, env_id: str = DEFAULT_MAZEEXPLORE_ENV_ID,
        ...
        # MiniHack-ExploreMaze's default (unlike MazeWalk's) is 8-way
        # movement PLUS open/kick/search PLUS eat
        # (minihack/envs/exploremaze.py's own ACTIONS, built from
        # minihack.envs.corridor.NAVIGATE_ACTIONS with an eat action
        # appended).
...
# Populated per-instance (see MiniHackMazeExploreEnv.__init__, and
...
# gets plain numeric labels ("0", "1", ...) until a MiniHackMazeExploreEnv
# has actually been constructed at least once.
ACTION_NAMES: dict[int, str] = {}


def play_minihack_mazeexplore() -> None:
    env = MiniHackMazeExploreEnv()
    observation, _ = env.reset()
    terminated = False
    truncated = False

    print(f"MiniHackMazeExploreEnv ({env.env_id})")
