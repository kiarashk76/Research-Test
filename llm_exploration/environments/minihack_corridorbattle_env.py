"""MiniHack-CorridorBattle family: a thin wrapper around MiniHack's own
...
# The lit variant -- MiniHack-CorridorBattle's only other variant (-Dark-)
# is the same fixed layout (a narrow corridor packed with several
# creatures the agent must get past to reach a goal position at the far
# end -- verified directly against the installed package's
# ``minihack/envs/fightcorridor.py`` source, not assumed) without the
# corridor being lit; pass e.g.
# ``env_id="MiniHack-CorridorBattle-Dark-v0"`` as an override to pick it
# without any code change here. See
# https://minihack.readthedocs.io/en/latest/envs/skill_hard/fightcorridor.html
# for the full list.
DEFAULT_CORRIDORBATTLE_ENV_ID = "MiniHack-CorridorBattle-v0"
...
# MiniHackFightCorridor.__init__ (minihack/envs/fightcorridor.py) uses one
# flat default (350) for both registered variants -- verified directly
# against the installed package's source, not assumed.
def default_max_episode_steps_for(env_id: str) -> int:
    """MiniHackFightCorridor's real default `max_episode_steps` -- same
    for every registered ``MiniHack-CorridorBattle-*`` id (see module
    comment above); ``env_id`` is accepted only so callers (e.g. the Setup
    page) can use this the same way as the Room family's version of this
    function, which does vary by id."""
    return 350
...
# identical across both CorridorBattle variants (lit/dark) for the same
# reason -- the description itself must not reveal what's actually in the
# corridor (a packed group of hostile creatures) or that walking into one
# attacks it, which has to come from the agent's own observations.
ENVIRONMENT_DESCRIPTION = (
    "A navigation task along a single corridor toward a marked exit at the far end. "
    "Other creatures may be present along the way. The layout, and anything else in "
    "it, must be discovered through interaction."
)
...
class MiniHackCorridorBattleEnv(BaseEnvironment):
    """Wraps any ``MiniHack-CorridorBattle-*`` task id as a plain
    ...
    readability -- game logic is entirely MiniHack's (including how it
    resolves moving into a creature as a melee attack; this wrapper
    doesn't add or touch a dedicated attack action).
    """

    metadata = {"render_modes": ["ansi", "human"]}

    def __init__(self, env_id: str = DEFAULT_CORRIDORBATTLE_ENV_ID,
        ...
        # (MiniHack-CorridorBattle doesn't override MiniHackNavigation's
        # default, so it's plain 8-way movement -- there's no dedicated
        # attack action; walking into a creature attacks it).
...
# Populated per-instance (see MiniHackCorridorBattleEnv.__init__, and
...
# gets plain numeric labels ("0", "1", ...) until a
# MiniHackCorridorBattleEnv has actually been constructed at least once.
ACTION_NAMES: dict[int, str] = {}


def play_minihack_corridorbattle() -> None:
    env = MiniHackCorridorBattleEnv()
    observation, _ = env.reset()
    terminated = False
    truncated = False

    print(f"MiniHackCorridorBattleEnv ({env.env_id})")
